...
class BaseRecognizer(BaseModel, metaclass=ABCMeta):
    ...
    @abstractmethod
    def extract_feat(self, inputs: torch.Tensor, point_frame_all: torch.Tensor,**kwargs) -> ForwardResults:
    ...
    def loss(self, inputs: torch.Tensor, point_frame_all: torch.Tensor,data_samples: SampleList,
             **kwargs) -> dict:
        ...
        """
        feats, loss_kwargs = \
            self.extract_feat(inputs,point_frame_all,
    ...
    def predict(self, inputs: torch.Tensor, point_frame_all: torch.Tensor,data_samples: SampleList,
                **kwargs) -> SampleList:
        ...
        Returns:
            ...
            following keys.
                ...
                - item (torch.Tensor): Classification scores, has a shape
                    (num_classes, )
        """
        feats, predict_kwargs = self.extract_feat(inputs,point_frame_all, test_mode=True)
    ...
    def _forward(self,
        ...
        Returns:
            ...
            or ``head`` forward.
        """
        feats, _ = self.extract_feat(inputs,point_frame_all, stage=stage)
        return feats
    ...
    def forward(self,
        ...
        """
        if mode == 'tensor':
            return self._forward(inputs,point_frame_all, **kwargs)
        if mode == 'predict':
            return self.predict(inputs,point_frame_all, data_samples, **kwargs)
        elif mode == 'loss':
            return self.loss(inputs,point_frame_all, data_samples, **kwargs)
        else:
