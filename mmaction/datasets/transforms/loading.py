...
class RawFrameDecode(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        cache = {}
        results_1 = results['frame_inds'] -1 
        for i, frame_idx in enumerate(results_1):
        ...
        frames_point_inds = frames_point_clip_offsets[:, None] + np.arange(len(results_1))[None, :] * 1
        ...
        results['point_frame_all'] = point_frame_all
        results['imgs'] = imgs
        results['original_shape'] = imgs[0].shape[:2]
        results['img_shape'] = imgs[0].shape[:2]
